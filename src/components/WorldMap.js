...
import { NY20_API_KEY, WORLD_MAP_URL, SATELLITE_POSITION_URL } from "../constants";

export const POSITION_API_BASE_URL = `/api/${SATELLITE_POSITION_URL}`;
...
const WorldMap = ({
  ...
  const fetchPositions = () => {
    ...
    return selectedSatellites.map((sat) => {
      const id = sat.satid;
      return fetch(`/api/${SATELLITE_POSITION_URL}/${id}/${latitude}/${longitude}/${altitude}/${duration * 60}&apiKey=${NY20_API_KEY}`)
